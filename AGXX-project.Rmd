...
```{bash}
wget -N https://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/000/017/085/GCA_000017085.1_ASM1708v1/GCA_000017085.1_ASM1708v1_genomic.fna.gz -O CP000730.fna.gz #downloading fna file for genome
wget -N https://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/000/017/085/GCA_000017085.1_ASM1708v1/GCA_000017085.1_ASM1708v1_genomic.gtf.gz -O CP000730.gtf.gz #downloading gtf - the gene annotation file for the genome
```

#Download sample fastq files from ENA into the orignials folder using wget
```{bash}
...
```{bash}
srun -p msc_appbio --pty /bin/bash # Ask for resources to run the job
conda create -n agxx
conda activate agxx
```
...
#Create a bash file and run the script for all the fastq files:
```{bash}
nano alignment.sh
...
#Ensure that the correct sam files are present and then create a bash script convert sam file to bam
```{bash}
nano sam_bam.sh
...
```{bash}
featureCounts -a CP000730.gtf.gz -o output_counts.txt ERR2713020.sorted.bam ERR2713021.sorted.bam ERR2713022.sorted.bam ERR2713023.sorted.bam ERR2713024.sorted.bam ERR2713025.sorted.bam

```

# or this and combine everything manually?
...
output_dir="results/feature_counts"

# Create counts file
for sample in "${samples[@]}"; do
    echo "Creating counts file $sample"
    featureCounts -p -t CDS -g ID -F GFF -a "reference_genome/CP000730.1.gff" -o "$output_dir/${sample}counts.txt" "$input_dir/${sample}.sorted.bam"
done
