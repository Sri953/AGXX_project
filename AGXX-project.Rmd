...
res_merged$Induced <- ifelse(res_merged$padj <= 0.05 & res_merged$log2FoldChange >= 0.6, TRUE, FALSE) #create a column for significant values 
...
Calculate number of transcripts that are induced and repressed, including fold change.

``` {r}
print(paste("Number of up-regulated transcripts:", sum(res_merged$Induced, na.rm=TRUE))) #number of transcript that are upregulated

print(paste("Number of downregulated transcripts:", sum(res_merged$Repressed, na.rm=TRUE))) #number of transcripts that are downregulated
...
ggplot(filtered_data,                                       #plot the MA plot with filtered_data
  ...
  geom_point(aes(colour = Regulon), 
             size = 2, alpha =1)+ #plot colours for significantly induced regulons.
  ...
  #Plot Significant points that are not already identified induced Regulons
  geom_point(data = filtered_data %>% 
             filter(Induced &                          #induced transcripts
  ...
  #Plot Repressed transcripts
  geom_point(data = filtered_data %>% 
             filter(Repressed),
             colour = "grey30", size = 2, alpha = 1) + #repressed transcripts coloured dark grey
  labs(
...
# Combine "Induced" and "Repressed" into a single significance column to plot
filtered_data$Significance <- ifelse(filtered_data$Induced == TRUE, "Upregulated",
                                     ifelse(filtered_data$Repressed == TRUE,
                                     "Repressed", "Not Significant"))
...
ggplot(filtered_data2, 
  ...
  scale_color_manual(values = c("Upregulated" = "red", "Repressed" = "blue", "Not Significant" = "gray")) +
  labs(
    title = "Volcano Plot for Upregulated Genes",     #Labels
    x = "Log2 Fold Change",
    y = "-Log10(P-value)"
  ) +
