...
```{r}
# Creating a subset of results with relevant selected columns; "USA300.numbers" and "Regulon from the merged results data frame
...
```{r}
# Grouping regulon data from the rlog data frame
regulon_df <- rlog_df %>%
group_by(Regulon) %>%
...
regulon_matrix <- regulon_df %>%
  column_to_rownames("Regulon") %>%   # "Regulon" column becomes row name in the new matrix
  as.matrix()
```
